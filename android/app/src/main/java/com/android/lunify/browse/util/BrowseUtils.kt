package com.android.lunify.browse.util
...
fun playOnlineTrack(fragment: Fragment, viewModel: BrowseViewModel, item: ExtractedContent, category: BrowseCategory) {
    ...
    val musicViewModel = androidx.lifecycle.ViewModelProvider(activity)[com.android.lunify.player.viewmodel.PlayerViewModel::class.java]
    
    // Create a temporary placeholder song to display on the player bar immediately
    val tempSong = Song(
        ...
        path = item.url, // Temporary URL until resolved
        albumArtUri = if (!item.thumbnailUrl.isNullOrEmpty()) Uri.parse(item.thumbnailUrl) else null
    )

    // Notify player bar to show and display loading spinner
    if (category == BrowseCategory.MUSIC) {
        musicViewModel.prepareOnlinePlayback(tempSong)
    }
    ...
    viewModel.resolveStreamingUrls(item.url) { videoUrl, audioUrl ->
        if (videoUrl.isNotEmpty()) {
            ...
            if (category == BrowseCategory.MUSIC) {
                ...
                val song = tempSong.copy(path = directAudioUrl)
            ...
            if (category == BrowseCategory.MUSIC) {
                musicViewModel.setResolvingStream(false)
