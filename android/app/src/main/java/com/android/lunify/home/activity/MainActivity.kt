package com.android.lunify.home.activity
...
import com.android.lunify.home.adapter.TabPagerAdapter
import com.android.lunify.player.sheet.PlayerSheetFragment
import com.android.lunify.auth.AuthViewModel
import com.android.lunify.home.viewmodel.HomeViewModel
import com.android.lunify.player.viewmodel.PlayerViewModel
...
class MainActivity : AppCompatActivity() {
    ...
    private val googleSignInLauncher = registerForActivityResult(
        ...
        try {
            ...
            val errorMessage = GoogleSignInErrorMapper.getErrorMessage(e)
        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            when (intent?.action) {
                ...
                MusicService.BROADCAST_PLAYBACK_STATE -> {
                    ...
                    playerViewModel.updatePlaybackState(isPlaying, song)
                }
    ...
    private val duoSyncReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            when (intent?.action) {
                com.android.lunify.duo.ui.activity.DuoSongsListActivity.ACTION_DUO_PLAY -> {
                    val song = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                        intent.getParcelableExtra(com.android.lunify.duo.ui.activity.DuoSongsListActivity.EXTRA_SONG, Song::class.java)
                    } else {
                        @Suppress("DEPRECATION")
                        intent.getParcelableExtra(com.android.lunify.duo.ui.activity.DuoSongsListActivity.EXTRA_SONG)
                    }
                    song?.let {
                ...
                com.android.lunify.duo.ui.activity.DuoSongsListActivity.ACTION_DUO_PAUSE -> {
                    android.util.Log.d("MainActivity", "DuoSongsListActivity pause - syncing with Duo")
                    duoViewModel.syncPause()
                }
                com.android.lunify.duo.ui.activity.DuoSongsListActivity.ACTION_DUO_RESUME -> {
    ...
    private fun initializeViewModel() {
        ...
        homeViewModel.initialize(repository)
    }
    ...
    private fun openEqualizer() {
        ...
        val intent = Intent(this, com.android.lunify.equalizer.ui.EqualizerActivity::class.java)
        startActivity(intent)
    }

    private fun openUserAgreement() {
        com.android.lunify.legal.LegalDocumentActivity.startUserAgreement(this)
    }

    private fun openPrivacyPolicy() {
        com.android.lunify.legal.LegalDocumentActivity.startPrivacyPolicy(this)
    }
    ...
    private fun setupPlayerBar() {
        ...
        playerBarBinding.btnPlayPause.setOnClickListener {
            val isCurrentlyPlaying = playerViewModel.isPlaying.value ?: false
    ...
    private fun observeViewModel() {
        ...
        playerViewModel.playSongEvent.observe(this) { song ->
            song?.let {
                ...
                playerViewModel.clearPlaySongEvent()
            }
        }
        ...
        playerViewModel.showPlayerBar.observe(this) { show ->
            binding.playerBarContainer.visibility = if (show) View.VISIBLE else View.GONE
        }

        playerViewModel.isResolvingStream.observe(this) { isResolving ->
            if (isResolving) {
                playerBarBinding.btnPlayPause.visibility = View.INVISIBLE
                playerBarBinding.pbLoading.visibility = View.VISIBLE
            } else {
                playerBarBinding.btnPlayPause.visibility = View.VISIBLE
                playerBarBinding.pbLoading.visibility = View.GONE
            }
        }

        homeViewModel.navigateToSongsList.observe(this) { data ->
        ...
        lifecycleScope.launch {
            duoViewModel.connectionState.collect { state ->
                val wifiDirectConnected = state is com.android.lunify.duo.data.model.DuoConnectionState.Connected
                isDuoConnected = wifiDirectConnected || duoViewModel.webRTCConnectionState.value is com.android.lunify.duo.webrtc.model.WebRTCConnectionState.Connected
        ...
        lifecycleScope.launch {
            duoViewModel.webRTCConnectionState.collect { state ->
                val webRTCConnected = state is com.android.lunify.duo.webrtc.model.WebRTCConnectionState.Connected
                isDuoConnected = webRTCConnected || duoViewModel.connectionState.value is com.android.lunify.duo.data.model.DuoConnectionState.Connected
    ...
    private fun updateDrawerHeader(user: com.android.lunify.data.model.User?) {
    ...
    private fun updateProgress(position: Int, duration: Int) {
        ...
        playerViewModel.updateProgress(position, duration)
    }
    ...
    private fun playSongNow(song: Song) {
        ...
        val playlist = playerViewModel.getCurrentPlaylist().ifEmpty { homeViewModel.songs.value ?: listOf(song) }
    ...
    private fun registerReceivers() {
        ...
        val duoSyncFilter = IntentFilter().apply {
            addAction(com.android.lunify.duo.ui.activity.DuoSongsListActivity.ACTION_DUO_PLAY)
            addAction(com.android.lunify.duo.ui.activity.DuoSongsListActivity.ACTION_DUO_PAUSE)
            addAction(com.android.lunify.duo.ui.activity.DuoSongsListActivity.ACTION_DUO_RESUME)
        }
    ...
    override fun onBackPressed() {
        if (binding.drawerLayout.isDrawerOpen(GravityCompat.START)) {
            ...
            val browseFragment = supportFragmentManager.findFragmentById(R.id.browseContainer) 
                as? com.android.lunify.browse.fragment.BrowseFragment
