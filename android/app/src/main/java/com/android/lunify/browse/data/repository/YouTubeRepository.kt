...
import com.android.lunify.browse.data.mapper.YouTubeMapper.toYouTubeChannel
import com.android.lunify.browse.data.mapper.YouTubeMapper.toYouTubePlaylist
    ...
    fun getHomeContent(): Flow<Result<YouTubeHomeContent>> = flow {
        try {
            ...
            if (trendingResponse.isSuccessful) {
                ...
                // Get lunify category videos for recommended
    ...
    fun getRelatedVideos(videoId: String): Flow<Result<List<YouTubeVideo>>> = flow {
        try {
            ...
            if (videoResponse.isSuccessful) {
                ...
                val searchQuery = video?.snippet?.title?.split(" ")?.take(3)?.joinToString(" ") ?: "lunify"
